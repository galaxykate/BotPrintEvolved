...
define(["common"], function(common) {
    ...
    var Follower = common.Transform.extend({
        init : function(edge, pct, offset, thetaOffset) {

            this._super();
            this.edge = edge;
            this.pct = pct;
            this.offset = thetaOffset ? thetaOffset : 0;
            this.thetaOffset = thetaOffset ? thetaOffset : 0;
            this.refresh();
        },

        refresh : function() {
            this.setTo(this.edge.start);
            this.addMultiple(this.edge.edge, this.pct);
            this.addMultiple(this.edge.normal, this.offset);
            this.rotation = this.edge.normal.getAngle() + this.thetaOffset;
        },

        toString : function() {
            return this.toSimpleString() + this.rotation.toFixed(2) + " edge: " + this.edge + " [" + this.pct.toFixed(2) + ", " + this.offset.toFixed(2) + "]";
        }
    });
    ...
    var Edge = Class.extend({
        ...
        getClosestPosition : function(target) {
            ...
            var found = {
                ...
                transform : this.createFollower(m, 0, 0),
            };
        ...
        createFollower : function(pct, offset, thetaOffset) {
            return new Follower(this, pct, offset, thetaOffset);
        },
