...
define(["ui", "./bot/tuning", "./bot/bot", "./physics/arena", "threeUtils", "./botEvo", "app", "common", "./population", "./scoreGraph", "./heuristic", "./bot/attachment/attachments"], function(UI, Tuning, Bot, Arena, threeUtils, BotEvo, App, common, Population, ScoreGraph, Heuristic, Attachment) {
	...
	var BotApp = App.extend({
		...
		initializeEditMode : function() {
			...
			this.makePartWindows();

		},

		intializePartList : function() {

		},

		//
		makePartWindows : function() {
			for (var i = 0; i < app.attachmentTypes.length; i++) {
				var canva = $("<canvas/>", {
					id : 'edit_item ' + i,
					class : 'edit_item',
					width : 150,
				...
				var canvases = document.getElementById('edit_item ' + i);
				if (canvases != null) {
					// Add processing to that element
					var processingInstance = new Processing(canvases, function(g) {
						// Do any setup stuff in here
						var attachment = new app.attachmentTypes[i];
						
						var lastIndex;
						var capitalCount = 0;
						var capitalIndexArray = [];
						var attachmentName = attachment.id;
						
						// put a space before uppercase letters
						for(var y = 0; y < attachmentName.length; y++){	 										
        					if('A' <= attachmentName.charAt(y) && attachmentName.charAt(y) <= 'Z'){	
        						attachmentName = attachmentName.substring(0, y) + " " + 
        							attachmentName.substring(y, attachmentName.legnth);
        						y++;
           					}
						}
						
						
						for(var t = 0; t < attachmentName.length; t++){
							if (!isNaN(attachmentName.charAt(t))) {
  								lastIndex = t - 1;
							}
						}
						
						attachmentName = attachmentName.substring(0,lastIndex);
						
						// Set what happens on each frame
						g.draw = function() {
							g.translate(70,60);
							g.scale(1.4,2.6);
							g.colorMode(g.HSB, 1);
