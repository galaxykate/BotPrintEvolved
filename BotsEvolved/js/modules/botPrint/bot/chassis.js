...
define(["common", "graph", "./wiring", "./tuning", "./attachment/attachments", "./component"], function(common, Graph, Wiring, Tuning, Attachment, Component) {'use strict';
	...
	var Chassis = common.Tree.extend({
		...
		generateAttachment : function(typeIndex) {
			...
			var weights = [.3];
			var attachmentTypes = [Attachment.Sensor];

			if(app.getOption("useThrusters")){
				attachmentTypes.push(Attachment.Actuator), weights.push(0.6);	
			}
			...
			if (app.getOption("useWheel")) {
				attachmentTypes.push(Attachment.Actuator.Wheel), weights.push(1);
			}
